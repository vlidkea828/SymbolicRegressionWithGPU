...
def calculate(x1, x2):
    """Does a calc"""
    q = cp.zeros((1, 8), dtype=cp.float32)
    q[0][0] = 11
    q[0][1] = 7
    q[0][2] = 0
    q[0][3] = 0
    ...
    r = cp.zeros((1, 8), dtype=cp.float32)
    r[0][0] = 3
    r[0][1] = 7
    r[0][2] = 0
    r[0][3] = 6
    ...
    z = q.shape[0] - 1
    print(z)
    y = cp.zeros((26941), dtype=cp.float32)
    stat_results_kernel((929,), (29,), (x1, x2, y, q, z)) 
    return y
...
def get_results(equation_string, variables_1, variables_2):
    ...
    variables_length = variables_1.shape[0]
    results = cp.zeros((variables_length), dtype=cp.float32)
    ...
    is_same = cp.array_equal(variables_2, results)
    return results
